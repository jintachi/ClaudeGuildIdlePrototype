...
func generate_random_quest():
	...
	quest_type = types[RNG.wrapper.randi() % types.size()]
	quest_rank = ranks[RNG.wrapper.randi() % ranks.size()]
...
func generate_requirements():
	...
	# Stat requirements
	min_total_health = int(50 * rank_multiplier * RNG.wrapper.randf_range(0.8, 1.2))
	min_total_defense = int(20 * rank_multiplier * RNG.wrapper.randf_range(0.8, 1.2))
	min_total_attack_power = int(30 * rank_multiplier * RNG.wrapper.randf_range(0.8, 1.2))
	min_total_spell_power = int(15 * rank_multiplier * RNG.wrapper.randf_range(0.8, 1.2))
	min_substat_requirement = int(quest_rank * RNG.wrapper.randf_range(0.5, 1.5))

func generate_rewards():
	var rank_multiplier = get_rank_multiplier()
	
	base_experience = int(20 * rank_multiplier)
	gold_reward = int(15 * rank_multiplier * RNG.wrapper.randf_range(0.8, 1.4))
	influence_reward = int(5 * rank_multiplier * RNG.wrapper.randf_range(0.9, 1.1))
	
	# Random additional rewards
	if RNG.wrapper.randf() < 0.3:
		building_materials = int(rank_multiplier * RNG.wrapper.randf_range(1, 3))
	if RNG.wrapper.randf() < 0.2:
		armor_pieces = int(rank_multiplier * RNG.wrapper.randf_range(1, 2))
	if RNG.wrapper.randf() < 0.2:
		weapons = int(rank_multiplier * RNG.wrapper.randf_range(1, 2))
	if RNG.wrapper.randf() < 0.4:
		food = int(rank_multiplier * RNG.wrapper.randf_range(2, 5))
...
func generate_quest_details():
	...
	description = descriptions[quest_type][RNG.wrapper.randi() % descriptions[quest_type].size()]
...
func complete_quest():
	...
	for character in assigned_party:
		...
		var individual_success = RNG.wrapper.randf_range(0,1)
...
func apply_injuries_on_failure(final_success_rate: float):
	...
	for i in range(assigned_party.size()):
		...
		if member_failed and RNG.wrapper.randf() < injury_chance:
...
func apply_random_injury(character: Character):
	...
	var injury_type = injury_types[RNG.wrapper.randi() % injury_types.size()]
